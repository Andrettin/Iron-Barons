...
Item {
	...
	Grid {
		...
		columns: Math.min(Math.floor((parent.width - min_spacing) / (portrait_width + min_spacing)), minister_offices.length)
		spacing: columns === minister_offices.length ? min_spacing : Math.max(min_spacing, Math.floor((parent.width - columns * portrait_width) / (columns + 1)))
		visible: minister_offices.length > 0
		
		readonly property int portrait_width: 64 * scale_factor + 2 * scale_factor
		readonly property int min_spacing: 16 * scale_factor
		
		Repeater {
			model: minister_offices
	...
	Grid {
		...
		anchors.horizontalCenter: parent.horizontalCenter
		width: columns * (portrait_width + spacing) - spacing
		columns: Math.min(Math.floor((parent.width - min_spacing) / (portrait_width + min_spacing)), offices.length)
		spacing: columns === offices.length ? min_spacing : Math.max(min_spacing, Math.floor((parent.width - columns * portrait_width) / (columns + 1)))
